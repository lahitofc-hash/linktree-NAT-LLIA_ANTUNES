...
import * as Icons from "lucide-react"; // Importa todos os ícones para busca dinâmica
...
interface LinkItem {
  label: string;
  url: string;
  icon: string; // Nome do ícone no Lucide (ex: Instagram, Music2, Youtube)
  highlight: string; // "true" ou "false"
}
...
export default function LinktreeArtista() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 flex flex-col items-center px-6 py-16 font-sans">
      ...
      {/* HEADER FIXO (Pode ser movido para a planilha depois) */}
      <motion.div 
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            links.map((link, index) => {
              // Busca o ícone dinamicamente. Se não existir, usa 'ExternalLink'
              // @ts-ignore
              const IconComponent = Icons[link.icon] || Icons.ExternalLink;
              const isHighlighted = link.highlight?.toLowerCase() === "true";
